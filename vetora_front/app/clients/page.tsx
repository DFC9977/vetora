...
export default function ClientsPage() {
  ...
  async function loadClients(search?: string) {
    setLoading(true);
    setError(null);
    const res = await api.lookupClients({ query: search ?? "" });
    setLoading(false);
    if (!res.ok) {
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-lg font-semibold">{t("clients.listTitle")}</h1>
        ...
        <div className="overflow-auto border rounded bg-white">
          <table className="min-w-full text-sm">
            <thead className="bg-slate-50">
              <tr>
                <th className="text-left px-3 py-2 border-b">{t("clients.listTitle")}</th>
              ...
              {clients.map((c) => (
                <tr key={c.id} className="hover:bg-slate-50">
                  <td className="px-3 py-2 border-b">
                    <a href={`/clients/${c.id}`} className="text-sky-700 hover:underline">
                      {c.name}
                    </a>
                  </td>
                  <td className="px-3 py-2 border-b">
                    {c.mobile || c.phone || <span className="text-slate-400">–</span>}
                  </td>
                  <td className="px-3 py-2 border-b">
